use crate::editor_old::editor_window::editor_window::EditorWindow;
...
impl winit::application::ApplicationHandler<UserEvent> for Editor {
  fn resumed(&mut self, event_loop: &winit::event_loop::ActiveEventLoop) {
    ...
    let engine = create_engine_instance(gl);
    ...
    let mut redraw = || {
      ...
      self
        ...
        .run(self.gl_window.as_mut().unwrap().window(), |egui_ctx| {
          self.frame.render_window(egui_ctx, &mut self.clear_color);
        });
